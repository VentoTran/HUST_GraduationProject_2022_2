...
{
    uint8_t temp_Bit2nd_Data = data & 0x02;
    uint8_t temp_Bit3rd_Data = data & 0x04;
    data &= ~0x06;
    data |= (uint8_t)((temp_Bit2nd_Data << 1) | (temp_Bit3rd_Data >> 1));

    uint32_t tempReg =  (uint32_t)DATA_PORT->ODR;
    tempReg &= 0xFFFFFF00;
    tempReg |= (uint32_t)data;
    DATA_PORT->ODR = tempReg;
